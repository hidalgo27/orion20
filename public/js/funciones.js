      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            ...
            Swal.fire('Error!', 'Hubo un error al borrar el producto.', 'danger');
          }
        }
      });
    }
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            Swal.fire('Borrado!', 'La asociacion ha sido borrada.', 'success');
            $('#row_lista_asociaciones_' + id).remove();
          } else if (data == 0) {
            Swal.fire('Error!', 'Hubo un error al borrar la asociacion.', 'danger');
          }
        }
      });
    }
...
function buscar_asociacion(ruc_rs) {
  ...
  if (valor.length > 0) {
    ...
    $.ajax({
      ...
      beforeSend: function beforeSend(data1) {
...
function agregar_precio(valor1, valor2) {
  ...
  var cadena = '<tr id="row_' + valor1 + '_precios_' + valor2 + '_' + valor + '">' + '<td>' + '<select class="form-control" name="categoria_n[]" id="categoria">' + '<option value="Nacional">Nacional</option>' + '<option value="Extranjero">Extranjero</option>';

  if (valor1 == 'a') {
    cadena += '<option value="Agencia">Agencia</option>';
  }

  cadena += '</select>' + '</td>' + '<td>' + '<input class="form-control" type="number" min="0" name="minimo_' + valor1 + '_n_' + valor2 + '[]" id="minimo">' + '</td>' + '<td>' + '<input class="form-control" type="number" min="0" name="maximo_' + valor1 + '_n_' + valor2 + '[]" id="maximo">' + '</td>' + '<td>' + '<input class="form-control" type="number" min="0" name="precio_' + valor1 + '_n_' + valor2 + '[]" id="precio">' + '</td>' + '<td>' + '<button class="btn btn-danger" type="button" onclick="borrar_precio(\'' + valor1 + '\',\'' + valor2 + '\',\'' + valor + '\')"><i class="fas fa-trash-alt"></i></button>' + '<button class="btn btn-success d-none" type="button" onclick="agregar_precio(\'' + valor1 + '\')"><i class="fas fa-plus"></i></button>' + '</td>' + '</tr>';
...
function enviar_datos_editar(valor1, valor2) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      $('#rpt_form_' + valor1 + '_' + valor2).html(data.mensaje);
      $('#rpt_form_' + valor1 + '_' + valor2).addClass(data.nombre_clase); // $("#form_"+valor1+'_'+valor2)[0].reset();
    }
  });
}
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            Swal.fire('Borrado!', 'El servicio ha sido borrada.', 'success');
            $('#servicio_' + id).fadeOut();
          } else if (data == 0) {
            Swal.fire('Error!', 'Hubo un error al borrar el servicio.', 'danger');
          }
        }
      });
    }
...
function confirmar(tipo_servicio, grupo_id, estado) {
  ...
  if (estado == 0) estado = 1;else if (estado == 1) estado = 0;
  console.log('tipo_servicio:' + tipo_servicio + ',grupo_id:' + grupo_id + ',estado:' + estado);
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (data.rpt == 1) {
        ...
        Swal.fire('Upps!', 'Hubo un error, vuelva a intentarlo.', 'error');
      }
    }
  });
}
...
function mostrar_provincias_servicios(departamento_id, categoria, categoria_id, producto_id) {
  ...
  if (departamento_id > 0) {
    ...
    $.ajax({
      ...
      success: function success(data) {
        $("select[name='provincia'").html('');
        $("select[name='provincia'").html(data.options); // $('#provincia_'+categoria_id+'_'+producto_id).html('');
...
function pasar_datos(clase, categoria_id, producto_id) {
  var resultArray = []; //    $("input[name='proveedor_id[]']").each(function () {
  ...
  resultArray = $("input[class='" + proveedor_id + "']").map(function () {
    return this.value;
  }).get();
  ...
  $("input[name='" + proveedor + "[]']").each(function (index) {
    ...
    if ($(this).is(':checked')) {
      ...
      if (resultArray == '') {
        var cadena = '<div id="lista_proveedores_saved_' + categoria_id + '_' + producto_id + '_' + valor[0] + '" class="row">' + '<div class="col-7 ">' + valor[1] + '</div>' + '<div class="col-3 px-0 mx-0"><input class="' + proveedor_id + '" type="hidden" name="proveedor_id_a[]" value="' + valor[0] + '"><input type="hidden" name="proveedor_id[]" value="' + valor[0] + '"><input class="form-control" type="number" name="precio_proveedor[]" min="0" step="0.01"></div>' + '<div class="col-2 px-0 mx-0"><button type="button" class="btn btn-danger" onclick="borrar_proveedor_save(\'' + categoria_id + '\',\'' + producto_id + '\',\'' + valor[0] + '\')"><i class="fas fa-trash"></i></button></div>' + '</div>';
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            ...
            Swal.fire('Opps!', 'El producto esta presente an algunas reservas, no borre este producto.', 'warning');
          }
        }
      });
    }
...
function escojer_proveedor(valor1, rol) {
  ...
  $.ajax({
    ...
    data: {
      transporte_externo_guia_id: valor1,
      proveedor_id_pago: proveedor_id_pago,
...
function agregar_hora(valor1) {
  ...
  var cadena = '<div id="hora_' + valor1 + '_' + valor + '" class="input-group mb-1">' + '<div class="input-group-prepend">' + '<button type="button" class="btn btn-secondary">Hora</button>' + '</div>' + '<select class="form-control" name="hora[]">' + '<option value="07:00:00">07:00:00</option>' + '<option value="07:30:00">07:30:00</option>' + '<option value="08:00:00">08:00:00</option>' + '<option value="08:30:00">08:30:00</option>' + '<option value="09:00:00">09:00:00</option>' + '<option value="09:30:00">09:30:00</option>' + '<option value="10:00:00">10:00:00</option>' + '<option value="10:30:00">10:30:00</option>' + '<option value="11:00:00">11:00:00</option>' + '<option value="11:30:00">11:30:00</option>' + '<option value="12:00:00">12:00:00</option>' + '<option value="12:30:00">12:30:00</option>' + '<option value="13:00:00">13:00:00</option>' + '<option value="13:30:00">13:30:00</option>' + '<option value="14:00:00">14:00:00</option>' + '<option value="14:30:00">14:30:00</option>' + '<option value="15:00:00">15:00:00</option>' + '<option value="15:30:00">15:30:00</option>' + '<option value="16:00:00">16:00:00</option>' + '<option value="16:30:00">16:30:00</option>' + '<option value="17:00:00">17:00:00</option>' + '</select>' + '<div class="input-group-prepend">' + '<button type="button" class="btn btn-danger" onclick="borrar_hora(\'' + valor1 + '\',\'' + valor + '\')"><i class="fas fa-trash-alt"></i> </button>' + '</div>' + '</div>';
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            ...
            Swal.fire('Error!', 'Hubo un error al borrar al administrador.', 'danger');
          }
        }
      });
    }
...
function confirmar_(tipo_servicio, grupo_id, estado, nuevo_estado) {
  console.log('tipo_servicio:' + tipo_servicio + ',grupo_id:' + grupo_id + ',estado:' + estado); // if(estado==0)
  //     estado=1;
  // else if(estado==1)
  //     estado=0;
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (data.rpt == 1) {
        ...
        $('#estado_span_' + tipo_servicio + '_' + grupo_id).removeClass('badge-dark');
        $('#estado_span_' + tipo_servicio + '_' + grupo_id).removeClass('badge-success');
        $('#estado_span_' + tipo_servicio + '_' + grupo_id).removeClass('badge-danger'); // $('#confirmar_'+tipo_servicio+'_'+grupo_id).removeClass('btn-primary');
        // $('#confirmar_'+tipo_servicio+'_'+grupo_id).removeClass('btn-danger');

        $('#estado_span_' + tipo_servicio + '_' + grupo_id).addClass(data.clase_span);
        $('#estado_span_' + tipo_servicio + '_' + grupo_id).html(data.estado_span); // $('#confirmar_'+tipo_servicio+'_'+grupo_id).addClass(data.clase_confirmar);
        // $('#confirmar_'+tipo_servicio+'_'+grupo_id).html(data.estado_confirmar);
      } else if (data.rpt == 0) {
        Swal.fire('Upps!', 'Hubo un error, vuelva a intentarlo.', 'error');
      }
...
function mostrar_pagina(grupo_id, estado) {
  // console.log('tipo_servicio:'+tipo_servicio+',grupo_id:'+grupo_id+',estado:'+estado);
  if (estado == 0) estado = 1;else if (estado == 1) estado = 0; // console.log('tipo_servicio:'+tipo_servicio+',grupo_id:'+grupo_id+',estado:'+estado);
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (data.rpt == 1) {
        $('#mostrar_en_pagina_' + grupo_id).val(data.estado); // $('#estado_span_'+tipo_servicio+'_'+grupo_id).removeClass('badge-dark');
        // $('#estado_span_'+tipo_servicio+'_'+grupo_id).removeClass('badge-success');
        // $('#estado_span_'+tipo_servicio+'_'+grupo_id).removeClass('badge-primary');
        // $('#estado_span_'+tipo_servicio+'_'+grupo_id).removeClass('badge-danger');
        // $('#estado_span_'+tipo_servicio+'_'+grupo_id).addClass(data.clase_span);
...
function es_correcto_fotos(mensa, archivo, ancho, alto) {
  ...
  if (o.files.length != 0) {
    if (!/\.(jpeg|jpg|png)$/i.test(foto.name)) {
      c = 1;
      Swal.fire('MENSAJE DEL SISTEMA', '' + mensa + ' ingrese una imagen con alguno de los siguientes formatos: .jpeg/.jpg/.png.', 'error');
      return false; //   alert('Ingrese una imagen con alguno de los siguientes formatos: .jpeg/.jpg/.png.');
    } else {
      var img = new Image();
      ...
      img.onload = function dimension() {
        if (this.width.toFixed(0) != ancho && this.height.toFixed(0) != alto) {
          c = 1;
          Swal.fire('MENSAJE DEL SISTEMA', '' + mensa + ' las medidas deben ser: (' + ancho + 'x' + alto + ' px). Medida de la foto que desea subir es: (' + this.width.toFixed(0) + 'x' + this.height.toFixed(0) + ' px)', 'error');
          return false; // alert('Las medidas deben ser: 900 x 400');
          // alert(c);
        } else {// Swal.fire(
            //     'MENSAJE DEL SISTEMA',
            //     'Imagen correcta :)',
...
function es_correcto_fotos_galeria(mensa, archivo, ancho, alto) {
  ...
  var fotos_erradas = 0; //manejaremos tipo de errors 1=formato, 2=medida
  ...
  if (foto.length != 0) {
    ...
    if (fotos_erradas > 0) {
      if (tipo_error == 1) {
        Swal.fire('MENSAJE DEL SISTEMA', '' + mensa + ' ingrese las imagenes con alguno de los siguientes formatos: .jpeg/.jpg/.png.' + '.Se subio (' + imagen_con_error + ').', 'error');
        return false;
      }
    }
    ...
    var procesaImg = function procesaImg(data) {
      var img = new Image();
    ...
    for (var i = 0; i < foto.length; i++) {
      procesaImg(foto[i]);
      console.log('nombre x:' + foto[i]); // var img = new Image();
      // // img.src = foto[i];
      // img.onload = function() {
      // // imgwidth = this.width;
    ...
    console.log('error:' + imagen_con_error); // if(fotos_erradas>0){
    //     if(tipo_error==2){
    //         Swal.fire(
    //             'MENSAJE DEL SISTEMA',
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            ...
            Swal.fire('Avertencia!', 'La categoria tiene actividaes relacionadas, modifique o borre las actividaes que tengan esta categoria.', 'danger');
          } else if (data == 0) {
            Swal.fire('Error!', 'Hubo un error al borrar la categoria.', 'danger');
          }
        }
      });
    }
      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          if (data == 1) {
            ...
            Swal.fire('Avertencia!', 'La marca tiene productos relacionados, modifique o borre los productos que tengan esta marca.', 'danger');
          } else if (data == 0) {
            Swal.fire('Error!', 'Hubo un error al borrar la marca.', 'danger');
          }
        }
      });
    }
      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          if (data == 1) {
            ...
            Swal.fire('Avertencia!', 'La unidad tiene productos relacionados, modifique o borre los productos que tengan esta marca.', 'danger');
          } else if (data == 0) {
            Swal.fire('Error!', 'Hubo un error al borrar la unidad.', 'danger');
          }
        }
      });
    }
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data == 1) {
            ...
            Swal.fire('Error!', 'Hubo un error al borrar al proveedor.', 'danger');
          } else if (data == 2) {
            Swal.fire('Opps!', 'El proveedor tiene productos, por favor borre sus productos e intentelo una vez más.', 'warning');
          }
        }
      });
    }
...
module.exports = __webpack_require__(/*! C:\Users\WorkStation\proyects\homestead\laravel\orion20\resources\js\funciones.js */"./resources/js/funciones.js");
